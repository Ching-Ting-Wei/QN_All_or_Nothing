#ifndef __MYALGO4_H
#define __MYALGO4_H

#include <iostream>
#include <algorithm>
#include <omp.h>
#include <queue>
#include <limits>
#include <string>
#include <cmath>

#include "../AlgorithmBase/AlgorithmBase.h"
#include "../../Request/WholeRequest.h"
#include "../../Network/Graph/Graph.h"
#include "../../config.h"

using namespace std;

class MyAlgo4:public AlgorithmBase {

private:
    //map<pair<int,int>, vector<double>> X;
    vector<map<pair<int,int>, double>> Y;
    vector<double> alpha;
    map<vector<int>, double> x_i_p;
    map<pair<int,int>, double> beta;
    vector<vector<vector<int>>> all_source_target_path;
    vector<vector<vector<int>>> comb;
    int change_edge_num = 0; 
    int diff_num = 0;
    double epsilon = 0.2;
    double obj = 0;
    double delta;
    double M;               
    double X(int u, int v, vector<double>&alpha , map<pair<int,int>,double>&beta );
public: 
    map<int, int> num_of_path_count;
    map<int, int> path_length_encode;
    map<int, int> path_length_cnt;
    vector<int> separation_oracle(int req_no, double &U, vector<double>&alpha , map<pair<int,int>,double>&beta );
    vector<int> Dijkstra(int src, int dst, int req_no);
    void path_assignment();
    void calculate();
    void entangle();
    void swap();
    void send();
    void dfs(int s, int t, vector<vector<int>> &ans, vector<int> &path, vector<bool> &visited);    
    vector<vector<int>> allPathsSourceTarget(int src, int dst);
    void combination(int index, int req_no, vector<int>&cur_comb);
    void next_time_slot();
    vector<map<vector<int>, int>> Greedy_rounding();
    void find_bottleneck(vector<int>, int req_no, bool obj_change, vector<double>&alpha , map<pair<int,int>,double>&beta );
    void initialize();
    void check_enough(vector<map<vector<int>, int>> &path);
    void readd(vector<map<vector<int>, int>> &path,vector<int> &over_memory,map<vector<int>,int> &over_channel);
    double changing_obj();
    void find_violate();
    vector<map<vector<int>, int>> rounding();
    MyAlgo4(string filename, int request_time_limit, int node_time_limit, double swap_prob, double entangle_alpha);
    MyAlgo4(string filename, int request_time_limit, int node_time_limit, double swap_prob, double entangle_alpha, double epsilon);
    ~MyAlgo4();
};

#endif
